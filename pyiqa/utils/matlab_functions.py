...
def imresize(img, scale, antialiasing=True):
    ...
    Args:
        img (Tensor | Numpy array):
            Tensor: Input image with shape (b, c, h, w), [0, 1] range.
    ...
    Returns:
        Tensor: Output image with shape (b, c, h, w), [0, 1] range, w/o round.
    """
    squeeze_flag = False
    ...
    # process H dimension
    # symmetric copying
    img_aug = torch.FloatTensor(in_b, in_c, in_h + sym_len_hs + sym_len_he, in_w)
    ...
    out_1 = torch.FloatTensor(in_b, in_c, out_h, in_w)
    kernel_width = weights_h.size(1)
    for b in range(in_b):
        for i in range(out_h):
            idx = int(indices_h[i][0])
            for j in range(in_c):
                out_1[b, j, i, :] = img_aug[b, j, idx:idx + kernel_width, :].transpose(0, 1).mv(weights_h[i])

    # process W dimension
    # symmetric copying
    out_1_aug = torch.FloatTensor(in_b, in_c, out_h, in_w + sym_len_ws + sym_len_we)
    ...
    out_2 = torch.FloatTensor(in_b, in_c, out_h, out_w)
    kernel_width = weights_w.size(1)
    for b in range(in_b):
        for i in range(out_w):
            idx = int(indices_w[i][0])
            for j in range(in_c):
                out_2[b, j, :, i] = out_1_aug[b, j, :, idx:idx + kernel_width].mv(weights_w[i])
