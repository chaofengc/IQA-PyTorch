...
class PieAPP(nn.Module): 
    ...
    def forward(self, dist, ref):
        assert dist.shape == ref.shape, f"Input and reference images should have the same shape, but got {dist.shape} and {ref.shape}"
        dist = self.preprocess(dist)
        ref = self.preprocess(ref)

        image_A_patches = extract_2d_patches(dist, self.patch_size, self.stride, padding='same') 
        image_ref_patches = extract_2d_patches(ref, self.patch_size, self.stride, padding='same') 
