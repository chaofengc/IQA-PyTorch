    ...
    try:
        ...
        if archive_path.suffix == '.zip':
            with zipfile.ZipFile(archive_path, 'r') as zip_ref:
                ...
                with tqdm(total=total_size, unit='B', unit_scale=True, desc="Extracting ZIP") as pbar:
        ...
        elif archive_path.suffix in ['.tar', '.tgz'] or archive_path.suffixes == ['.tar', '.gz']:
            with tarfile.open(archive_path, 'r:*') as tar_ref:
                # Get total size for progress bar
                total_size = sum(member.size for member in tar_ref.getmembers())
                extracted_size = 0
                
                with tqdm(total=total_size, unit='B', unit_scale=True, desc="Extracting TAR/TGZ") as pbar:
